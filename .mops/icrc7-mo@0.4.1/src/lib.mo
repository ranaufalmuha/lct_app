...
module {
  ...
  public class ICRC7(stored : ?State, canister : Principal, environment : Environment) {
    ...
    public func get_token_infos(token_ids : [Nat]) : [(Nat, ?NFT)] {
      ...
      let result = Array.map<Nat, (Nat, ?NFT)>(
        token_ids,
        func(x : Nat) : (Nat, ?NFT) {
          switch (get_nft(x)) {
            case (null) (x, null);
    ...
    private func mint<system>(thisItem : SetNFTItemRequest, bMinting : Bool, bNew : Bool) : SetNFTResult {
      ...
      let ?metadataIndex = Vec.firstIndexWith<(Text, Value)>(
        innerMap,
        func(x : (Text, Value)) : Bool {
          x.0 == "meta";
        },
      ) else {
            ...
            switch (val.meta) {
              case (#Class(props)) {
                  ...
                  case (?remote_func) {
                    switch (remote_func<system>(txMap, ?txTopMap, preNotification)) {
                      ...
                      case (#err(tx)) {
                        Vec.add(
                          results,
                          #Err(#GenericError({ error_code = 6453; message = tx })),
                        );
                        continue proc;
                      };
                    };
                ...
                let #Map(innerMapArray) = finaltx else {
                  Vec.add(
                    results,
                    #Err(#GenericError({ error_code = 6453; message = "canMint did not return a tx map" })),
                  );
                  continue proc;
                };
                ...
                let ?metadataIndex = Vec.firstIndexWith<(Text, Value)>(
                  ...
                  Vec.add(
                    results,
                    #Err(#GenericError({ error_code = 6453; message = "canMint did not return a meta tag" })),
                  );
                  continue proc;
                };
    ...
    public func update_token_owner(token_id : Nat, previous_owner : ?Account, target_owner : Account) : Result.Result<NFT, Error> {
      ...
      if (not validAccount(target_owner)) return #err({
        message = "invalid account " # debug_show (target_owner);
        error_code = 394845;
      });
